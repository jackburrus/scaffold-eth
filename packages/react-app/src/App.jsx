...
function App(props) {
  ...
  const onNearestX = (value, { index }) => {
    setCrosshairValues([
      { x: value.x, y: new Intl.NumberFormat("en-US", { maximumSignificantDigits: 5 }).format(value.y) },
    ]);
  };
  ...
  if (data) {
    inkGraph = (
      <>
        <XYPlot xType="time" width={400} height={300} onMouseLeave={onMouseLeave}>
          <LineSeries data={transformedData} onNearestX={onNearestX} curve="curveMonotoneX" color="blue" />
    ...
    inkGraph = <pre>Error: {error.message}. Is your subgraph ready?</pre>;
  }
  ...
  const artistColumns = [
    {
      ...
      key: "id",
      render: text => (
        <a href={"https://nifty.ink/artist/" + text} target="_blank">
          {text}
        </a>
      ),
    },
        ...
        <Switch>
          <Route exact path="/">
            <div style={{ width: 400, margin: "auto", marginTop: 32 }}>
              ...
              {data ? (
                <>
                  <Title>Most prolific artists</Title>
                  <Table dataSource={data.artists} columns={artistColumns} />
                </>
              ) : (
                <></>
              )}
          ...
          <Route path="/graphiql">
            <div style={{ height: 500, marginTop: 32, textAlign: "left" }}>
              <GraphiQL fetcher={graphQLFetcher} docExplorerOpen query={EXAMPLE_QUERY} />
            </div>
          </Route>
          <Route path="/peepeth">
            <div style={{ height: 500, marginTop: 32, textAlign: "left" }}>
